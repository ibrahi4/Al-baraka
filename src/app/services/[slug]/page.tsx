...
import { 
  ...
  Clock, Users, Award, ChevronLeft, MapPin
} from "lucide-react";
    ...
    <>
        ...
        <div className="relative container-custom py-12 md:py-16">
          <div className="grid lg:grid-cols-2 gap-8 lg:gap-12 items-center">
            <div className="text-center lg:text-right">
              ...
              <h1 className="text-3xl md:text-4xl lg:text-5xl font-black mb-5 leading-tight">{service.name}</h1>
              <p className="text-base md:text-lg text-white/80 leading-relaxed mb-8 max-w-xl mx-auto lg:mx-0">{service.shortDescription}</p>
              <div className="flex flex-col sm:flex-row gap-3 justify-center lg:justify-start">
                ...
                <Button asChild size="lg" variant="outline" className="bg-white/10 hover:bg-white/20 border-white/30 text-white hover:text-white h-12 px-8 backdrop-blur">
                  <a href={`https://wa.me/${siteConfig.whatsapp}`} target="_blank" rel="noopener noreferrer"><MessageCircle className="w-4 h-4 ml-2" />طلب عرض سعر</a>
                </Button>
              </div>
            </div>
                ...
                <CardContent className="p-8">
                  ...
                  <div className="space-y-4">
                    {[
                      ...
                      { icon: Users, label: "فرق محترفة" },
                      { icon: Award, label: "خبرة 10+ سنوات" },
                    ].map((item, i) => (
      ...
      <section className="section-padding bg-white">
        <div className="container-custom">
          <div className="max-w-3xl mx-auto text-center mb-10 md:mb-14">
            ...
            <p className="text-sm md:text-base text-gray-600 leading-relaxed">نلتزم بأعلى معايير الجودة في كل خطوة من خطوات تقديم الخدمة</p>
          </div>
      ...
      <section className="section-padding bg-[#F8F6F2]">
        <div className="container-custom">
          <div className="text-center max-w-2xl mx-auto mb-10">
            ...
            <h2 className="text-2xl md:text-3xl font-black text-[#1B2A41] mb-4">نقدم خدمة {service.name} في جميع محافظات مصر</h2>
          </div>
          <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-3">
            {governorates.slice(0, 12).map((g) => (
              <Link key={g.slug} href={`/areas/${g.slug}`}>
                <Card className="hover:border-[#C9A961] hover:shadow-md transition-all group cursor-pointer border-gray-200">
                  <CardContent className="p-4 flex items-center gap-3">
                    <div className="w-10 h-10 bg-[#1B2A41]/5 group-hover:bg-[#C9A961] text-[#1B2A41] group-hover:text-white rounded-lg flex items-center justify-center transition-all">
                      <MapPin className="w-5 h-5" />
                    </div>
                    <div className="font-bold text-sm text-[#1B2A41] group-hover:text-[#C9A961] transition-colors">{g.name}</div>
                  </CardContent>
                </Card>
              </Link>
            ))}
      ...
      <section className="section-padding bg-white">
        <div className="container-custom max-w-3xl">
          <div className="text-center mb-10">
            ...
            <h2 className="text-2xl md:text-3xl lg:text-4xl font-black text-[#1B2A41] mb-4">إجابات لأكثر الأسئلة شيوعاً</h2>
          </div>
      ...
      <section className="relative bg-gradient-to-br from-[#1B2A41] via-[#0F1A2B] to-[#1B2A41] text-white overflow-hidden">
        ...
        <div className="relative container-custom py-12 md:py-16 text-center">
          <h2 className="text-2xl md:text-4xl font-black mb-5">احصل على عرض سعر <span className="text-[#C9A961]">مجاني</span></h2>
          <p className="text-white/70 mb-8 max-w-2xl mx-auto">تواصل معنا الآن لمناقشة احتياجاتك والحصول على استشارة مجانية</p>
          <div className="flex flex-col sm:flex-row justify-center gap-3">
            <Button asChild size="lg" className="bg-[#C9A961] hover:bg-[#A8893F] text-white h-12 px-8 shadow-xl">
              <a href={`tel:${siteConfig.phone}`}><Phone className="w-4 h-4 ml-2" />اتصل الآن</a>
            </Button>
            <Button asChild size="lg" variant="outline" className="bg-white/10 hover:bg-white/20 border-white/30 text-white hover:text-white h-12 px-8 backdrop-blur">
              <a href={`https://wa.me/${siteConfig.whatsapp}`} target="_blank" rel="noopener noreferrer"><MessageCircle className="w-4 h-4 ml-2" />واتساب</a>
            </Button>
          </div>
        </div>
      </section>
