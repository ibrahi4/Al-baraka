...
export default function AreasPage() {
  ...
  const AreaCard = ({
    ...
    <motion.div
      initial={{ opacity: 0, y: featured ? 30 : 20 }}
      whileInView={{ opacity: 1, y: 0 }}
      viewport={{ once: true }}
      transition={{ delay: index * (featured ? 0.1 : 0.05) }}
    >
      <Link href={`/areas/${area.slug}`}>
        {featured ? (
              ...
              {area.compounds && area.compounds.length > 0 && (
                <div className="flex flex-wrap gap-1.5 mb-4">
                  {area.compounds.slice(0, 3).map((c) => (
                    <span
                      key={c}
                      className="text-[10px] bg-[#C9A961]/10 text-[#C9A961] px-2 py-0.5 rounded-full font-medium"
                    >
                      {c}
                    </span>
                  ))}
          ...
          <Card className="h-full hover:border-[#C9A961] hover:shadow-md transition-all group cursor-pointer border-gray-200 bg-white relative">
            {area.isVip && (
              <Badge className="absolute top-2 left-2 bg-[#C9A961] text-white border-0 text-[10px] z-10">
            ...
            <CardContent className="p-4 md:p-5 text-center">
              <div
                className={`w-10 h-10 md:w-12 md:h-12 ${
                  area.isVip
                    ? "bg-[#C9A961]/10 group-hover:bg-[#C9A961] text-[#C9A961] group-hover:text-white"
                    : "bg-[#1B2A41]/5 group-hover:bg-[#C9A961] text-[#1B2A41] group-hover:text-white"
                } rounded-lg flex items-center justify-center mx-auto mb-3 transition-all`}
              >
              ...
              <div className="text-[10px] md:text-xs text-gray-500 mt-1">
                {area.isVip ? "نقل أثاث VIP" : "نقل أثاث"}
              </div>
            </CardContent>
          </Card>
        )}
        ...
        <div className="relative container-custom py-12 md:py-16 lg:py-20">
          <div className="max-w-3xl mx-auto text-center">
            ...
            <p className="text-base md:text-lg text-white/80 leading-relaxed mb-8">
              متخصصون في نقل الأثاث داخل القاهرة والجيزة والمدن الجديدة والكمبوندات الحديثة
            </p>
            <div className="relative max-w-xl mx-auto">
              <Search className="absolute right-4 top-1/2 -translate-y-1/2 w-5 h-5 text-gray-400" />
              <Input
                type="text"
                placeholder="ابحث... (مدينة نصر، التجمع، Hyde Park، مدينتي...)"
                value={searchQuery}
        ...
        <section className="section-padding bg-white">
          <div className="container-custom">
            <div className="mb-8">
              ...
              <p className="text-gray-600">نتائج بحثك عن &quot;{searchQuery}&quot;</p>
            </div>
            {filteredResults.length > 0 ? (
              <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-3 md:gap-4">
                {filteredResults.map((area, i) => (
        ...
        <>
            ...
            <div className="container-custom">
              <div className="text-center max-w-2xl mx-auto mb-10 md:mb-12">
                ...
                <h2 className="text-2xl md:text-3xl lg:text-4xl font-black text-[#1B2A41] mb-3">
                  متخصصون في{" "}
                  <span className="text-[#C9A961]">المدن الجديدة والكمبوندات</span>
                </h2>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 md:gap-6">
                {vipAreas.map((area, i) => (
                  <AreaCard key={area.slug} area={area} index={i} featured />
                ))}
              </div>
            </div>
          </section>
          ...
          <section className="section-padding bg-[#F8F6F2]">
            <div className="container-custom">
              <div className="flex items-center gap-3 mb-8">
                <div className="w-10 h-10 bg-[#1B2A41] text-[#C9A961] rounded-xl flex items-center justify-center">
              ...
              <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-3 md:gap-4">
                {areaGroups.cairo.areas.map((area, i) => (
                  <AreaCard key={area.slug} area={area} index={i} />
                ))}
              </div>
            </div>
          ...
          <section className="section-padding bg-white">
            <div className="container-custom">
              ...
              <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-3 md:gap-4">
                {areaGroups.giza.areas.map((area, i) => (
                  <AreaCard key={area.slug} area={area} index={i} />
                ))}
              </div>
            </div>
          ...
          <Card className="bg-gradient-to-br from-[#1B2A41] to-[#0F1A2B] border-0 text-white overflow-hidden relative">
            ...
            <CardContent className="p-8 md:p-12 text-center relative">
              <h2 className="text-2xl md:text-3xl font-black mb-4">
                منطقتك مش موجودة؟{" "}
                <span className="text-[#C9A961]">تواصل معنا</span>
              </h2>
              ...
              <div className="flex flex-col sm:flex-row justify-center gap-3">
                <Button
                  asChild
                  size="lg"
                  className="bg-[#C9A961] hover:bg-[#A8893F] text-white h-12 px-8"
                >
                  <a href={`tel:${siteConfig.phone}`}>
                    <Phone className="w-4 h-4 ml-2" />
                    اتصل الآن
